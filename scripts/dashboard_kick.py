...
def get_connection():
    log_security_event("DB_CONNECT", "Opening PostgreSQL connection for Kick dashboard")
...
def get_kick_momentum(conn, days=7, limit=15):
    query = """
        WITH latest_run AS (
...
def make_chart(df):
    ...
    fig = px.bar(
        ...
        title="Top Kick Categories by Current Viewers"
    )

    fig.update_layout(
        height=360,
        template="plotly_dark",
        paper_bgcolor="#061a0b",
        plot_bgcolor="#061a0b",
...
def build_dashboard(days=7):
    log_security_event("KICK_DASHBOARD_BUILD", "Building Kick dashboard")

    conn = get_connection()
    df = get_kick_momentum(conn, days=days, limit=15)
    conn.close()
...
<html>
<head>
<title>Project2026 Kick Trends</title>
<style>
body {{
    background:
        radial-gradient(circle at top left, rgba(83,252,24,.35), transparent 30%),
        linear-gradient(135deg, #020d04, #061a0b 50%, #0b2a10);
    color: #ecffe8;
...
th,
td {{
    border-bottom: 1px solid #1f7a2a;
    padding: 10px;
    text-align: left;
}}
...
<h1>Project2026 Kick Trends</h1>
<p class="subtitle">Alternative livestreaming trends based on Kick viewers, stream counts, and category momentum.</p>
...
<div class="summary">
    <div class="summary-card">
        ...
        <span>Categories / Games Tracked</span>
    </div>

    <div class="summary-card">
        <strong>{top_game}</strong>
        <span>Top Kick Category</span>
    </div>
...
"""
    ...
    with open("dashboard_kick.html", "w", encoding="utf-8") as f:
        f.write(html)

    print("Created dashboard_kick.html")
    log_security_event("KICK_DASHBOARD_SUCCESS", "Created dashboard_kick.html")
